...
export function MaintenanceLog() {
  ...
  const { videoRef, start, stop, active, error } = useBarcodeScanner((value) => {
    ...
    if (asset) {
      setAssetId(asset.id);
      setScanMessage(`Найден актив: ${asset.name}`);
    ...
    if (part) {
      ...
      setScanMessage(`Добавлена позиция склада: ${part.name}`);
      setScannerOpen(false);
      return;
    }
    setScanMessage(`Код «${value}» не найден в реестре оборудования или на складе.`);
  });
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle>Журнал работ по инженерным системам</CardTitle>
        </div>
        <CardDescription>
          Наряд на работу (ИТП, насосная, ГРЩ и т.д.) со списанием материалов со склада и
          автоматической записью в «План/факт», если указана статья сметы.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-5">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 p-3 text-sm dark:border-slate-800">
            <span className="text-slate-500">
              {assetsWithoutQr.length > 0
                ? `${assetsWithoutQr.length} единиц оборудования без QR-этикетки`
                : "У всего оборудования есть QR-этикетки"}
            </span>
            <button
            ...
            >
              <Printer className="h-3.5 w-3.5" /> {printBusy ? "Формирование…" : "Печать шильдиков"}
            </button>
          </div>
        )}
        ...
        <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
          <div className="mb-3 flex items-center gap-2">
            <h4 className="text-sm font-semibold">Новый наряд</h4>
            {scannerSupported ? (
              <button
                onClick={() => {
                  setScannerOpen((v) => !v);
              ...
              >
                <ScanLine className="h-3.5 w-3.5" /> {scannerOpen ? "Закрыть сканер" : "Сканировать QR"}
              </button>
            ) : (
          ...
          {scannerOpen && (
            <div className="mb-3 rounded-lg border border-emerald-300 bg-emerald-50/60 p-3 dark:border-emerald-800 dark:bg-emerald-950/30">
              <div className="flex items-center gap-2 text-xs text-slate-500">
                <Camera className="h-3.5 w-3.5" /> Наведите камеру на QR-этикетку оборудования или позиции склада
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <label className="flex flex-col gap-1 text-xs text-slate-500">
              ...
              <select value={workType} onChange={(e) => setWorkType(e.target.value as MaintenanceWorkType)} className="h-9 rounded-lg border border-slate-300 bg-white px-2 text-sm dark:border-slate-700 dark:bg-slate-900">
                {WORK_TYPES.map((t) => (
                  <option key={t} value={t}>{MAINTENANCE_WORK_TYPE_LABELS[t]}</option>
                ))}
              </select>
            </label>
            <label className="flex flex-col gap-1 text-xs text-slate-500">
              Техник
              <input value={technicianName} onChange={(e) => setTechnicianName(e.target.value)} placeholder="ФИО" className="h-9 rounded-lg border border-slate-300 bg-white px-2 text-sm dark:border-slate-700 dark:bg-slate-900" />
            </label>
            <label className="flex flex-col gap-1 text-xs text-slate-500">
              Оборудование
          ...
          <label className="mt-3 flex flex-col gap-1 text-xs text-slate-500">
            Описание работ
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} placeholder="Например: замена сальника циркуляционного насоса, опрессовка контура" className="rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm dark:border-slate-700 dark:bg-slate-900" />
          </label>
          ...
          <div className="mt-3 flex flex-col gap-2 rounded-lg border border-slate-100 p-3 dark:border-slate-800">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-slate-500">Списанные материалы</span>
              ...
              <div className="flex items-center gap-2">
                <select value={materialToAdd} onChange={(e) => setMaterialToAdd(e.target.value)} className="h-8 flex-1 rounded-md border border-slate-200 bg-transparent px-1.5 text-xs dark:border-slate-700">
                  {project.spareParts.map((p) => (
                    <option key={p.id} value={p.id}>{p.name} (остаток {p.quantityOnHand} {p.unit})</option>
                  ))}
                </select>
          ...
          <label className="mt-3 flex flex-col gap-1 text-xs text-slate-500">
            Статья сметы для списания (необязательно — тогда попадёт в «План/факт»)
          ...
          >
            <Plus className="h-4 w-4" /> Зафиксировать наряд
          </button>
        </div>

        <div className="flex flex-col divide-y divide-slate-100 dark:divide-slate-800">
          {recentLogs.length === 0 && (
            <p className="py-6 text-center text-sm text-slate-400">Журнал пуст — зафиксируйте первый наряд выше.</p>
          )}
          {recentLogs.map((log) => {
