...
export interface OwnershipUnit {
  ...
  unitType: UnitType;
  /**
   * Адрес дома/корпуса — обязателен к заполнению, если реестр одного проекта
   * охватывает несколько корпусов/адресов (напр. импорт ведомости ЕРЦ по
   * управляющей организации, обслуживающей несколько домов): без него
   * нумерация квартир «1, 2, 3…» разных корпусов неотличима друг от друга.
   * Не путать с BuildingProfile.address — тот один на проект, этот — на
   * конкретное помещение реестра.
   */
  address?: string;
  ...
  documentRef?: string;
  notes?: string;
  /** Метка источника записи — проставляется при создании/обновлении импортом сальдовой ведомости ЕРЦ */
  source?: string;
  /** Лицевой счёт (ЛС) из ведомости ЕРЦ — ключ сверки при повторном импорте */
  personalAccount?: string;
  /** Задолженность по ТО лифтов на конец периода последней ведомости, ₸ (отрицательное — переплата/аванс) */
  debtElevatorKzt?: number;
  /** Задолженность по эксплуатационным расходам на конец периода последней ведомости, ₸ */
  debtOperationalKzt?: number;
  /** Начисление за период ведомости по ТО лифтов, ₸ — база для оценки числа месяцев долга */
  monthlyChargeElevatorKzt?: number;
  /** Начисление за период ведомости по эксплуатационным расходам, ₸ */
  monthlyChargeOperationalKzt?: number;
  /** Расчётный период последней импортированной ведомости, напр. «08/2026» */
  debtPeriod?: string;
  /** Когда последний раз обновлялись долговые данные импортом ведомости, ISO */
  debtImportedAt?: string;
  createdAt: string;
  updatedAt: string;
}
