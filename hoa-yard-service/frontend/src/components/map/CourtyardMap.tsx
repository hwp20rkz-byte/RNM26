...
import { MapPin, Trash2, Trees, DoorOpen, Dumbbell, Baby, Waypoints } from "lucide-react";
import type { Zone, ZoneType } from "@/types/domain";
import { YARD_CENTER, buildings, zones } from "./zones.data";
...
export function CourtyardMap({ onZoneSelect }: CourtyardMapProps) {
  ...
  const polygonZones = useMemo(() => zones.filter((z) => z.geometryType === "POLYGON"), []);
  const pointZones = useMemo(() => zones.filter((z) => z.geometryType === "POINT"), []);
  ...
  return (
    <div className="relative h-full w-full bg-surface">
        ...
        {polygonZones.map((zone) => (
          <GeoJSON
            ...
            onEachFeature={(_feature, layer: Layer) => {
              layer.bindTooltip(zone.name, { sticky: true, className: "!text-xs" });
            }}
          />
        ))}
        ...
        {pointZones.map((zone) => {
          ...
          return (
            <CircleMarker
              ...
              pathOptions={ZONE_STYLE[zone.zoneType]}
      ...
      <p className="sr-only">
        Здания: {buildings.map((b) => b.address).join(", ")}
      </p>
    </div>
  );
}
